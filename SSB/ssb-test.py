...
print(current_directory) # /home/postgres/my-ssb
...
configures = [
...

    {
        ...
        '_mlsql_set' : current_directory + '/queries',
        '_active': True,
        'ssb_test'         : 'on',
        'enable_logicml'   : 'on',
        'enable_physicml'  : 'on',
...
for configure in configures:
    ...
    conn = psycopg2.connect("dbname=test-ssb user=postgres password=yourpasswd host=127.0.0.1 port=5432")

    for filename in files:
        if filename[-4:] != '.sql': continue

        # obtain testing data
        with open(configure['_mlsql_set'] + '/' + filename, "r") as f:
